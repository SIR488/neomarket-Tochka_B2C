...
async def test_breadcrumbs_return_path_from_root(client, mock_b2b_client):
    response = await client.get("/api/v1/breadcrumbs?category_id=child-1")
    assert response.status_code == 200
    data = response.json()
    assert len(data["data"]) >= 2
    assert data["data"][0]["level"] == 1  # корень
...
async def test_ambiguous_params_returns_400(client):
    ...
    assert "ambiguous" in response.json().get("message", "").lower()


@pytest.mark.asyncio
async def test_orphan_node_returns_422(client, mock_b2b_client):
    mock_b2b_client.get_categories.return_value = [
        {"id": "child-1", "name": "Сирота", "parent_id": "non-existent"},
    ]
