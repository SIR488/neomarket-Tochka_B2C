...
async def test_similar_returns_up_to_8_from_same_category(client, mock_b2b_client):
    mock_b2b_client.get_similar_products.return_value = {
        "items": [
            {"id": "p1", "name": "Похожий 1", "price": 1500},
            {"id": "p2", "name": "Похожий 2", "price": 2500},
        ],
        "total": 12
    }

    response = await client.get(
        "/api/v1/products/123e4567-e89b-12d3-a456-426614174000/similar",
    ...
    assert all(item["id"] != "123e4567-e89b-12d3-a456-426614174000" for item in data["items"])
...
async def test_unknown_product_returns_404(client, mock_b2b_client):
    ...
    response = await client.get("/api/v1/products/11111111-1111-1111-1111-111111111111")

    assert response.status_code == 404
    data = response.json()
    assert data["code"] in ("NOT_FOUND", "PRODUCT_NOT_FOUND")